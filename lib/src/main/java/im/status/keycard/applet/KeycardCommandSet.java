...
public class KeycardCommandSet {
  ...
  public APDUResponse sendCommand(byte cmd, byte p1, byte p2, byte[] data) throws IOException {
    APDUCommand command = new APDUCommand(0x80, cmd, p1, p2, data);
    return apduChannel.send(command);
  }
  ...
  public APDUResponse select(int instanceIdx) throws IOException {
    APDUCommand selectApplet = new APDUCommand(0x00, 0xA4, 0x20, 0, Identifiers.getKeycardInstanceAID(instanceIdx));
